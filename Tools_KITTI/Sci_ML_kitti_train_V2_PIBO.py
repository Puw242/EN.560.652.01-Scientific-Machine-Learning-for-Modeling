...
with a PIBO (Physical-Information-Based Optimization) stage.

Pipeline:
  Stage 0: Physical-Information-Based Initialization (PIBO)
      - Use detection scores (Car_score, Ped_score, Cyc_score) as
        physical signals to pre-optimize the network.
      - No labels are used in this stage.

  Stage 1: Supervised Training
      - Use standard cross-entropy on 3-way labels for
        (Car, Pedestrian, Cyclist) heads.

Case file format (one JSON per frame):
...
BATCH_SIZE     = 16
LEARNING_RATE  = 1e-3
EPOCHS_STAGE0  = 5   # PIBO stage epochs
EPOCHS_STAGE1  = 50  # Supervised stage epochs # 30 for previous
VAL_RATIO      = 0.2  # 20% for validation

# Optionally, you can add L_phys into supervised loss with a small weight
USE_PHYS_IN_STAGE1 = False
LAMBDA_PHYS_STAGE1 = 0.1

# MLP architecture
INPUT_DIM     = 15   # 3 classes * 5 features each
HIDDEN_SIZES  = [64, 64, 32]  # 5-layer: input -> 3 hidden -> output
    ...
    def _load_all_cases(self):
        ...
        for path in all_files:
            if path.endswith(".json"):
                ...
                # .jsonl: assume first non-empty line is one case
                with open(path, "r", encoding="utf-8") as f:
                    for line in f:
                        s = line.strip()
                        if not s:
...
def physical_loss(feats: torch.Tensor,
    ...
    """
    Physical-Information-Based loss L_phys:
    Aligns output probabilities with detection scores (Car_score, Ped_score, Cyc_score).

    feats:  [B, 15]
        index:
          Car: [0]=present, [1]=score, [2]=iou, [3]=det_idx, [4]=diff
          Ped: [5]=present, [6]=score, [7]=iou, [8]=det_idx, [9]=diff
          Cyc: [10]=present, [11]=score, [12]=iou, [13]=det_idx, [14]=diff

    logits: [B, 3, 3]
        head 0: Car  (classes: None, PartA2, Other)
        head 1: Ped  (classes: None, PartA2, Other)
        head 2: Cyc  (classes: None, PartA2, Other)
    """
    probs = F.softmax(logits, dim=-1)  # [B, 3, 3]

    # Scores
    car_score = feats[:, 1]   # [B]
    ped_score = feats[:, 6]   # [B]
    cyc_score = feats[:, 11]  # [B]

    # Present flags
    car_present = feats[:, 0]   # 0 or 1
    ped_present = feats[:, 5]
    cyc_present = feats[:, 10]

    # Clamp to [0,1]
    car_score = car_score.clamp(0.0, 1.0)
    ped_score = ped_score.clamp(0.0, 1.0)
    cyc_score = cyc_score.clamp(0.0, 1.0)

    # Car: use probability of class=1 (PartA2) to align with car_score
    car_mask = car_present > 0.5
    if car_mask.any():
        car_diff = probs[car_mask, 0, 1] - car_score[car_mask]
        loss_car = (car_diff ** 2).mean()
    else:
        loss_car = torch.tensor(0.0, device=feats.device)

    # Ped: class=1 (PartA2) align with ped_score
    ped_mask = ped_present > 0.5
    if ped_mask.any():
        ped_diff = probs[ped_mask, 1, 1] - ped_score[ped_mask]
        loss_ped = (ped_diff ** 2).mean()
    else:
        loss_ped = torch.tensor(0.0, device=feats.device)

    # Cyc: class=2 (Other) align with cyc_score
    cyc_mask = cyc_present > 0.5
    if cyc_mask.any():
        cyc_diff = probs[cyc_mask, 2, 2] - cyc_score[cyc_mask]
        loss_cyc = (cyc_diff ** 2).mean()
    else:
        loss_cyc = torch.tensor(0.0, device=feats.device)

    return loss_car + loss_ped + loss_cyc
...
def train_one_epoch_phys(model: nn.Module,
                         ...
                         device: str) -> float:
    """
    Stage 0: only use physical loss L_phys (no labels).
    """
    model.train()
    total_loss = 0.0
    total_batches = 0
...
def train_one_epoch_sup(model: nn.Module,
    ...
    """
    Stage 1: supervised training with CE. Optionally add L_phys as regularizer.
    """
    model.train()
    total_loss = 0.0
    total_batches = 0
    ...
    for feats, labels in dataloader:
        ...
        loss_sup = loss_car + loss_ped + loss_cyc
...
def main():
    ...
    # ---------- Stage 0: Physical-Information-Based Initialization ----------
    ...
    for epoch in range(1, EPOCHS_STAGE0 + 1):
        ...
        # 用当前参数在 val 上跑一个 CE+Acc (作为“proxy”，看物理预优化对分类的副作用)
    ...
    # Save model
    save_path = "/home/frank/Pu/sci_ML/Tools/case_mlp_3way_pibo.pth"
    torch.save(model.state_dict(), save_path)
    print(f"\n[Done] PIBO Model saved to: {save_path}")
